---
layout: home
title: MicroBin Documentation
description: Documentation for MicroBin Corporation's products, solutions, and platforms

hero:
  name: MicroBin Documentation
  text: Your complete resource for MicroBin products
  tagline: Find documentation for hardware, IoT Dashboard, and PlatformQ
  image:
    src: /logo_gradient.png
    alt: MicroBin Logo
  actions:
    - theme: brand
      text: Hardware Docs
      link: /hardware/
    - theme: alt
      text: IoT Dashboard
      link: /iot-dashboard/
    - theme: alt
      text: PlatformQ
      link: /platformq/
    - theme: alt
      text: Contributing
      link: /contributing

features:
  - icon: 📟
    title: Hardware Products
    details: Documentation for MicroBin's commercial off-the-shelf hardware products
    link: /hardware/
  - icon: 📊
    title: IoT Dashboard
    details: Learn how to use the IoT dashboard builder and access API documentation
    link: /iot-dashboard/
  - icon: 🧠
    title: PlatformQ
    details: AI-enabled design & development platform for OEMs and SMBs
    link: /platformq/

---
::: tip Latest Updates
Check out our latest hardware release, the [Ex6eeder XP2](/hardware/ex6eeder)!
:::

## Quick Links

- [Hardware API Reference](/hardware/api)
- [IoT Dashboard Getting Started](/iot-dashboard/getting-started)
- [PlatformQ Account Creation](/platformq/account-creation)

## Support Resources

If you can't find what you're looking for in the documentation, you can:
- [Submit a support ticket](https://support.microbin.io)
- [Join our community forum](https://community.microbin.io)
- [Contact sales for product inquiries](https://microbin.io/contact)
